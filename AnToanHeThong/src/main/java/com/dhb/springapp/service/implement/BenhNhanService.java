...
public class BenhNhanService extends GenericService<BenhNhan> implements IBenhNhanService {
    ...
    public void themBenhNhanVaPhieuKhamBenh(AddPatient addPatient, SimpleDateFormat format) throws Exception {
        ...
        if (addPatient.getId() != null && !addPatient.getId().isEmpty() &&
            addPatient.getAppointmentID() != null && !addPatient.getAppointmentID().isEmpty()) {
            benhNhan = getById(BenhNhan.class, addPatient.getId());
            phieuKhamBenh = iPhieuKhamBenhService.getById(PhieuKhamBenh.class, Integer.parseInt(addPatient.getAppointmentID()));
        }
        else {
        ...
        if (!benhNhanRepository.themBenhNhanVaPhieuKhamBenh(benhNhan, phieuKhamBenh, addPatient.getLoaiBenhList()))
    ...
    public void themBenhNhanTaiKham(AddPatient addPatient, SimpleDateFormat format) throws Exception {
        ...
        if (patient != null) {
            ...
            if (!benhNhanRepository.themBenhNhanVaPhieuKhamBenh(patient, phieuKhamBenh, addPatient.getLoaiBenhList()))
                throw new Exception("Giao tác thêm thất bại");
        }
        else {
            // Them thong tin benh nhan
    ...
    public List<BenhNhan> getBenhNhanTheoSDT(String phone) {
        return benhNhanRepository.getBenhNhanTheoSDT(phone);
    }

    @Override
    public List<BenhNhan> getBenhNhanTheoTenVaSDT(String ten, String phone) {
        return benhNhanRepository.getBenhNhanTheoTenVaSDT(ten, phone);
    }

    @Override
    public List<BenhNhan> castPersistenceToTransient(List<BenhNhan> benhNhanList) {
        List<BenhNhan> result = new ArrayList<>();
        benhNhanList.forEach(b -> {
            BenhNhan benhNhan = new BenhNhan();
            benhNhan.setId(b.getId());
            benhNhan.setTen(b.getTen());
            benhNhan.setHo(b.getHo());
            benhNhan.setTuoi(b.getTuoi());
            benhNhan.setNgaySinh(b.getNgaySinh());
            benhNhan.setGioiTinh(b.getGioiTinh());
            benhNhan.setEmail(b.getEmail());
            benhNhan.setDienThoai(b.getDienThoai());
