...
public class AdminController {
    ...
    public String addProcess(@ModelAttribute("admin") @Valid AddEmployee admin,
                             ...
                             ModelMap model, @RequestParam(value = "lang", required = false)String lang) {
        if (!result.hasErrors()) {
            if(iTaiKhoanService.checkPassword(admin)) {
                if (iTaiKhoanService.checkExistedUsername(admin)) {
                    try {
                        ...
                        iTaiKhoanService.themTaiKhoanVaAdmin(path, admin);
                        return String.format("redirect:/admin/admin-management%s", str);
                    }
                    catch (Exception e) {
